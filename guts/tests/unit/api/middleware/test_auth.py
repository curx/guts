...
import guts.api.middleware.auth
from guts import test


class TestGutsKeystoneContextMiddleware(test.TestCase):

    def setUp(self):
        super(TestGutsKeystoneContextMiddleware, self).setUp()

        @webob.dec.wsgify()
        def fake_app(req):
            self.context = req.environ['guts.context']
            return webob.Response()

        self.context = None
        self.middleware = (guts.api.middleware.auth
                           .GutsKeystoneContext(fake_app))
